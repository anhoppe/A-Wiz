...
namespace Awiz.Core
{
    class ArchitectureSequenceView : ArchitectureView
    {
        ...
        public override void AddClassNode(ClassInfo classInfo)
        {
            ...
            bool isMethodCallInProgress = SequenceNodeGenerator.CreateClassNode(classInfo);
        ...
        public override void Load(IVersionUpdater versionUpdater)
        {
            ...
            Stack<CallInfo> callStack = new();

            if (FileSystem.Exists(storagePath))
            {
                using (var fileStream = FileSystem.OpenRead(storagePath))
                {
                    callStack = StorageAccess.LoadSequenceCallstack(fileStream, SequenceNodeGenerator.NodeToClassInfo);
                }
            }
        ...
        public override void Save()
        {
            ...
            using (var fileStream = FileSystem.Create(storagePath))
            {
                StorageAccess.SaveSequenceCallstack(new Stack<CallInfo>(), fileStream);
            }
        ...
        private void AddMethodCallReturn()
        {
            ...
            var (previousMethodCall, returnedMethodCall) = SequenceNodeGenerator.CreateReturnCall();
            InteractionBehavior.RemoveButtons(returnedMethodCall);

            if (previousMethodCall != null)
            {
                InteractionBehavior.UpdateButtons(previousMethodCall, (nextTargetClass, calledMethod) => 
                    AddMethodCall(previousMethodCall.TargetClass, nextTargetClass, calledMethod), AddMethodCallReturn);
            }
            else
            {
                if (SequenceNodeGenerator.UserLifeline == null)
